...
var upload = {
	...
	productSmallImg : multer({	dest : './public/img/small/'}),
	productBigImg : 	multer({	dest : './public/img/big/'}),
	productSlide1 : 	multer({	dest : './public/img/slide1/'}),
	productSlide2 : 	multer({	dest : './public/img/slide2/'}),
	productSlide3 : 	multer({	dest : './public/img/slide3/'}),
	productSlide4 : 	multer({	dest : './public/img/slide4/'}),
}
...
appServer.post('/deleteProduct', function(req, res) {
	'use strict';
	var	key,
			pathDeleteImg,
			id,
	...
	id = req.body.id;
	typeProduct = req.body.productType;
	nameShortDb = "shortDb_" + typeProduct;
	models[typeProduct].findById(id, function(err, doc) {
		if (err) {
			...
			allImgJSON = JSON.stringify(doc.img)
			doc.remove(function(err, removed) {
				if (err) {
					...
					//delete from short database
					models[nameShortDb].findOne({"id_db" : id},  function(err, obj) {
						if (err) {
							throw err
							// mail
							res.send('пусто')
							...
							obj.remove(function(err, removed) {
								if (err) {
									throw err;
									// mail
								}
									...
									//delete all images
									allImg = JSON.parse(allImgJSON);
									for(key in allImg) {
										pathDeleteImg = './public/img/' + String(key) + "/" + String(allImg[key]);
										fs.unlink(String(pathDeleteImg), function (err) {
											if (err) throw err;
											// mail
										});
...
appServer.post('/setProductDb', function(req, res) {
	'use strict'
	var 	product,
			productDb,
			commentsDb,
...
// upload images admin
appServer.post('/setImg_small', upload.productSmallImg.single('file'), function(req, res) {
	updateDbImg(req, res)
});
appServer.post('/setImg_big',   upload.productBigImg.single('file'), function(req, res) {
	updateDbImg(req, res)
});
appServer.post('/setImg_slide1', upload.productSlide1.single('file'), function(req, res) {
	updateDbImg(req, res)
});
appServer.post('/setImg_slide2', upload.productSlide2.single('file'), function(req, res) {
	updateDbImg(req, res)
});
appServer.post('/setImg_slide3', upload.productSlide3.single('file'), function(req, res) {
	updateDbImg(req, res)
});
appServer.post('/setImg_slide4', upload.productSlide4.single('file'), function(req, res) {
	updateDbImg(req, res)
});
